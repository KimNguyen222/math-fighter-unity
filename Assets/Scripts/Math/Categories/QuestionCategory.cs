    ...
    public abstract class QuestionCategory
    {
        ...
        protected void AddLevel(Level level)
        {
            if (_levels.Contains(level))
                Debug.Log("Trying to add a duplicate level to a category (cat:" + _catName + "/level:" + level.LevelName + ")");
            else
                _levels.Add(level);
        }
        ...
        protected void CreateAndAddLevel(int levelNum)
        {
            switch (levelNum)
            {
                ...
                default:
                    //Debug.Log("Level out of range: " + levelNum);
                    CreateAndAddLevel_1();
                    break;
            }
        }
        ...
        {
            if (!(_levels.Count == GetUniqueLevelCount() && levelNum < _levels.Count))
                Debug.Log("Level index is out of range: (" + levelNum + " vs " + _levels.Count + ")");
        ...
        {
            if (_levels.Count == 0)
                Debug.Log("Trying to get a question when there are no levels currently set");
            ...
            else
            {
                ...
                level = _levels[rnd.Next(0, GetUniqueLevelCount())];
            }
            ...
            if (level.CanRandomizeAnswerOrder())
            {
                ...
                question.RightAnswer = rnd.Next(0, 4);;
